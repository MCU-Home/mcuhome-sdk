...
| `device_pins.py` | The `sources:` block CI's firmware builds pin their device with — exactly the packages the run provides, read out of the source directories' `index.json` — and the check that the finished build resolved them. Without it a device that pins nothing is resolved under the workbench's default SDK constraint, and a local directory holding a version outside it is passed over for the package registry |
