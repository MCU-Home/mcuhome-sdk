...
wire format between the orchestrator that writes a build context and
the build environment that builds it, and the environment side runs on
what the SDK package declares as its runtime — a bare Python
interpreter, stdlib and nothing else. Every module the SDK entry point
imports, transitively, must hold to that, which is exactly the
constraint this package is built under (dependency-free by
construction) and exactly the constraint ``mcuhome.workbench`` is not:
its supported surface parses YAML, so importing it pulls a third-party
parser onto a path that only ever reads JSON.
``tests/python/test_container_closure.py`` pins the consequence — the
entry point's import closure stays stdlib plus ``mcuhome``.
...
def read_model(path: Path) -> DeviceModel:
    ...
    The receiving end of the pipeline ``docs/design/builder-pipeline.md``
    section 3 lays out: the model is the wire format of a remote build,
    so a build server is handed one of these and runs the generation and
    compilation stages on it. It deliberately re-runs nothing — the
    configuration tree, the secrets file and the whole front half of the
    pipeline stay on the machine that owns them, which is also why a
    build server never needs to be trusted with them.
