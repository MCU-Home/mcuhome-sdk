...
| `CMakeLists.txt` | The Matter build glue, **byte-identical to what the builder emits** — CHIP module registration, the link-order fix, the app-common include and `chip_configure_data_model()`. `tests/python/test_generate.py` asserts the two never diverge: a mechanism found on the bench must not end up in only one of them. |
